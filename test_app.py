...
app = FastAPI(
    title="AI Personal Finance Advisor - TEST VERSION",
    description="Simplified test version without external dependencies",
    version="1.0.0"
)
...
async def read_root():
    """Root endpoint"""
    return {
        "message": "AI Personal Finance Advisor API - TEST VERSION",
        "version": "1.0.0",
        "status": "running",
        "note": "This is a test version without external APIs",
        "endpoints": {
            "health": "/health",
...
@app.post("/api/v1/test/financial-advice")
async def test_financial_advice(query: str):
    """Test financial advice endpoint"""
    return {
        "success": True,
        "advice": f"Based on your query: '{query}', here's my AI-powered financial advice: Consider building an emergency fund of 3-6 months of expenses, diversify your investments across different asset classes, and regularly review your budget to identify savings opportunities.",
        "timestamp": asyncio.get_event_loop().time()
    }
